...
def normalize_one_line(text: str) -> str:
    """Colapsa quebras de linha e múltiplos espaços em uma linha única."""
...
def build_linux_text(instr: str, ginput: str, command: str) -> str:
    header = (
        "Você é uma assistente de Linux/terminal extremamente crítica em segurança "
        "e boas práticas.\n"
        "[DOMÍNIO: LINUX]\n"
    )
    full = (
        f"{header}Pergunta do usuário: {instr}\n"
        f"Contexto: {ginput}\n"
...
def build_crypto_text(row: Dict) -> str:
    ...
    Corrige erro "Timestamp is not JSON serializable" convertendo
    todos os valores não primitivos para string.
    """
    safe_row = {}
    for k, v in row.items():
        # Tipos simples vão direto
        if isinstance(v, (int, float, str)) or v is None:
            safe_row[k] = v
        else:
            # Timestamps, Decimals, numpy types etc -> string
            safe_row[k] = str(v)
...
def load_python_humaneval(limit: Optional[int] = None):
    ...
    def _format(e):
        p = e.get("prompt", "") or ""
        s = e.get("canonical_solution") or e.get("solution", "") or ""
...
def load_crypto_btc_usdt(limit: Optional[int] = None, month_split: Optional[str] = None):
    ...
    if month_split is None:
        # Usa o train inteiro (mas depois limit_per_domain corta)
        ds = load_dataset("WinkingFace/CryptoLM-Bitcoin-BTC-USDT", split="train")
    else:
        # Se quiser um arquivo parquet específico (não obrigatório)
        ds = load_dataset(
            "WinkingFace/CryptoLM-Bitcoin-BTC-USDT",
            split=f"train[{month_split}]",
        )
...
def parse_args():
    ...
    # Crypto split (opcional; se None usa o train inteiro)
    p.add_argument("--crypto_month_split", type=str, default=None,
                   help="Split do CryptoLM a usar, ex: 2024-01.parquet. Se omitido, usa o train todo.")
...
def main():
    ...
    training_args = TrainingArguments(
        ...
        bf16=args.bf16,
        logging_steps=10,
        eval_strategy="epoch",      # <<< SUA VERSÃO DO TRANSFORMERS USA ESTE NOME
        save_strategy="epoch",
