...
- Tokeniza em __getitem__ e usa collator com padding seguro.
...
class DataCollatorForCausalLM:
    ...
    def __call__(self, features: List[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
        # features aqui têm *apenas* tensores int64 (input_ids, attention_mask, labels)
...
def normalize_one_line(text: str) -> str:
    """Colapsa espaços/quebras de linha para evitar textos gigantes em uma linha."""
...
# =========================================================
# 4. Carregamento dos datasets HF (sem Excel)
# =========================================================

def load_python_datasets(limit_per_domain: int | None = None):
    """
...
def load_linux_datasets(limit_per_domain: int | None = None):
    """
    shikhardadhich/linux_commands -> columns: Instruction, Input, Output.
    """
...
def build_full_text_dataset(tokenizer, max_length: int, limit_per_domain: int | None = None):
    """
    ...
    print(
        f"Python samples: {ds_py.num_rows} | Linux samples: {ds_linux.num_rows}"
    )
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=200,
        help="Máximo de exemplos por domínio (Python / Linux). Use valores pequenos para testes.",
    )
